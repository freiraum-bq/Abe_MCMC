...
def post_mean_etas(draws):
    """
    Compute the per-customer posterior mean of the spending-rate (η) from level-1 draws.
    """
    all_draws = np.concatenate(draws["level_1"], axis=0)
...
# --- correlations between λ, μ, η from level-2 draws ----------------------
def extract_tri_correlations(draws_level2):
    """
    Compute 2.5%, 50%, and 97.5% quantiles of the correlations between
    purchase rate (λ), dropout rate (μ), and spending rate (η) from
    level-2 variance/covariance draws.
    """
    var_lambda = draws_level2[:, 0]
    var_mu     = draws_level2[:, 1]
    var_eta    = draws_level2[:, 2]
    cov_lm     = draws_level2[:, 3]
    cov_le     = draws_level2[:, 4]
    cov_me     = draws_level2[:, 5]

    corr_lm = cov_lm / np.sqrt(var_lambda * var_mu)
    corr_le = cov_le / np.sqrt(var_lambda * var_eta)
    corr_me = cov_me / np.sqrt(var_mu     * var_eta)

    return {
        "corr_lambda_mu":  np.percentile(corr_lm, [2.5, 50, 97.5]).round(2),
        "corr_lambda_eta": np.percentile(corr_le, [2.5, 50, 97.5]).round(2),
        "corr_mu_eta":     np.percentile(corr_me, [2.5, 50, 97.5]).round(2)
    }
