...
# %% 2. Load CDNOW CBS with customer covariates ---------------------------
# -- 2. Load CDNOW CBS with customer covariates ---------------------------
...
# %% 3. Running MCMC for model estimation M1 (no covariates)
# -- 3. Running MCMC for model estimation M1 (no covariates)
start_m1 = time.time()
draws_3pI = mcmc_draw_parameters_rfm_m(
    cal_cbs    = cbs_df,
    covariates = None,      # intercept‐only
    mcmc       =    8000,      # 8000 total iterations
    burnin     =    20000,   # discard the first 20 000 as warm-up
    thin       =    1,         # keep every draw; 8 000 draws after burn-in
    chains     =    4,
    seed       =    42,
    trace      =    1000,
    n_mh_steps =    50,
...
with open(os.path.join(pickles_dir, "ext_tri_m1.pkl"), "wb") as f:
...
pickles_dir = os.path.join(project_root, "outputs", "pickles")
os.makedirs(pickles_dir, exist_ok=True)
with open(os.path.join(pickles_dir, "ext_tri_m2.pkl"), "wb") as f:
...
for model_name, runtime in [
    ("ext_tri_M1", m1_duration),
    ("ext_tri_M2", m2_duration)
]:
